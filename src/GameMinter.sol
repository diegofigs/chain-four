...
import {StringUtils} from "StringUtils/StringUtils.sol";
import {Strings} from "@openzeppelin/utils/Strings.sol";
import {ConnectFour} from "connect4-sol/ConnectFour.sol";

/// @title Connect 4 Game Minter
/// @author Miguel Piedrafita, modified by Diego Figueroa
...
contract GameMinter is ERC721("Connect 4", "CONN4"), StringUtils {
    ...
    function mint(uint256 gameId) public {
        (address player1, address player2,, bool finished) = connect4.getGame(gameId);
        bool didPlayer1Win = connect4.didPlayerWin(gameId, 0);
        if (!finished || msg.sender != (didPlayer1Win ? player1 : player2)) revert CannotMintGame();
    ...
    function drawBoard(uint256 id) internal view returns (string memory) {
        (uint64 firstBoard, uint64 secondBoard) = connect4.getBoards(id);
        ...
        for (uint256 rowNum = 0; rowNum < 7; rowNum++) {
            ...
            for (uint256 colNum = 0; colNum < 6; colNum++) {
                ...
                if ((firstBoard >> bbPos) % 2 == 0 && (secondBoard >> bbPos) % 2 == 0) continue;
                ...
                columns[colNum] = string(
                    abi.encodePacked(
                        ...
                        '" r="45" fill="',
                        (firstBoard >> bbPos) % 2 == 0 ? "#22d3ee" : "#f472b6",
                        '"></circle>'
                    )
                );
            }
        ...
        encoded = Base64.encode(abi.encodePacked(encoded, rows[3], rows[4], rows[5], rows[6], "</svg>"));
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        // there is an issue in the original contract when encoding player name, this line fixes that
        string memory player = Strings.toHexString(uint256(uint160(winner)), 20);
